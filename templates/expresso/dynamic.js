var errortext = '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>{%$locale.47%}</div>';
var regtext = '<center><div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>{%$locale[48]%}</div></center>';
...
var timeout = 3000;
	...
	function  myTags(mytags){
		...
		var flashvars = {
			...
			tcolor2:"0x000000",{%*/*цвет 2 цвета мен€ютс€ от tcolor к tcolor2*/*%}
			hicolor:"0xB12AC8",{%*/*цвет тэгов при наведении курсора мыши*/*%}
			tspeed:"110",{%*/*скорость вращени€ облака*/*%}
			distr:"true",{%*/*true - распредел€ть тэги равномерно по поверхности сферы, false - не распредел€ть*/*%}
			mode:"tags",{%*/* o_O */*%}
			tagcloud:mytags
...
$('a.comm_edit').click(function(event) {
    var cont = $(this).parent().parent().parent().children(".comm_text");
	$.ajax({
    type: "GET",
...
$('#submit_login').click(function(event){
    ...
    $("#error").html('<img src="{%$settings.path%}images/ajax-loader5.gif" width="128" height="15" />');
    $("#error").show(500);
    ...
    type: "POST",
     ...
     success: function(html){
	     if(html == 'error')
		     ...
		     {
		     $("#block-user").hide(500);
	         $("#block-user").html("<script>document.location.href='{%$settings.path%}';</script>");
	         $("#block-user").show(500);
		     }
...
$("#mainimg").animate({ opacity: 0.01 }, {duration:500, complete: function(){$(this).attr('src', mainimgsrc)} });
$("#mainimg").animate({ opacity: 1 }, "slow");
event.preventDefault();
...
$('a.fav').click(function(event){
    ...
    var counter = parseInt($('#notepad-counter').html());
	$.ajax({
    type: "GET",
     ...
     success: function(html){
     	...
     	if(html == '1')
			{
			el.attr('class', 'glyphicon glyphicon-star-empty');
            $('#notepad-counter').html(counter - 1);
			//$('#'+eid).html('{%$locale.228%}');
			}
     	if(html == '2')
			{
		      $('#notepad-counter').html(counter + 1);
			el.attr('class', 'glyphicon glyphicon-star');
			//$('#'+eid).html('{%$locale.695%}');
			}
		}
	});
